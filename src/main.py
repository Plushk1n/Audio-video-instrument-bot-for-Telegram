...
def robust_input(src: str, **kwargs):
    """Вход: мягкая устойчивость к битым потокам + декод в 1 поток (память контейнера, защита от OOM).

    Число потоков декодера влияет только на память и скорость, но не на качество результата.
    На контейнере с лимитом памяти многопоточный декод тяжёлого видео (10-бит/HDR) выделяет
    буферы кадров на каждое ядро и упирается в лимит → процесс убивает OOM (rc=-9). Один поток
    это предотвращает; для коротких роликов скорость декода остаётся приемлемой.
    """
    return ffmpeg.input(src, err_detect="ignore_err", threads=1, **kwargs)
